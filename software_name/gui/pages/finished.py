...
class FinishedPage(ttk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        footerFrame = Footer(self, controller, False, "",
                             "", "FilterStandardsPage", "FinishedPage")

        doneLabel = ttk.Label(contentFrame, text="Done.",
                              font=controller.bigFont)

        outInfoFrame = ttk.Frame(contentFrame)
        nameTitleLabel = ttk.Label(
            outInfoFrame, text="Name of results: ", font=controller.mediumFont)
        # use a StringVar so the Label updates when the user changes the file name
        outNameStr = controller.frames[self.get_input_output_page(
        )].output_filename
        nameLabel = ttk.Label(
            outInfoFrame, textvariable=outNameStr, font=controller.mediumFont)
        locationLabel = ttk.Label(
            outInfoFrame, text="Location of results: ", font=controller.mediumFont)
        ...
        outPathLabel = ttk.Label(
            outInfoFrame, textvariable=outDirStr, font=controller.mediumFont)

        openButton = Button(contentFrame, text="Open",
                            command=lambda: self.open())
        showInFolderButton = Button(
            contentFrame, text="Show in folder", command=lambda: self.showInFolder())
        startAgainButton = Button(
            contentFrame, text="Start again", command=lambda: controller.show_frame("StartPage"))
        backButton = Button(contentFrame, text="Back",
