...
class InputOutputPage(ttk.Frame):
    ...
    def __init__(self, parent, controller, title):
        ...
        self.headerFrame = Header(
            self, controller, "2. Input / Output for " + title, "")
        contentFrame = Content(self, controller)
        self.footerFrame = Footer(
            self, controller, True, "Back", "Continue", "StartPage", "FilterStandardsPage")

        inputTitleLabel = ttk.Label(
            contentFrame, text="Input CSV files (Click 'open' to add files)", style='subtitle.TLabel', padding='0 0 0 5')

        inputListFrame = ttk.Frame(contentFrame)
        self.inputList = tk.Listbox(inputListFrame, listvariable=self.input_listbox_items,
                                    selectmode=EXTENDED, height=5, relief="flat")  # ridge
        scrollBar = ttk.Scrollbar(
            inputListFrame, orient=VERTICAL, command=self.inputList.yview)
        self.inputList.configure(yscrollcommand=scrollBar.set)

        self.errorLabel = ttk.Label(
            contentFrame, text="Unrecognised files will be ignored", style="error.TLabel", padding='0 0 5 0')

        inputButtonFrame = ttk.Frame(contentFrame, padding='0 5 0 15')
        removeButton = Button(inputButtonFrame, text="Remove",
                              command=self.removeSelectedInputFiles)
        openButton = Button(inputButtonFrame, text="Open",
                            command=self.addInputFile)

        outputTitleLabel = ttk.Label(
            contentFrame, text="Output Excel workbook", style='subtitle.TLabel', padding='0 0 0 5')

        self.outputPathLabel = ttk.Label(
            contentFrame, anchor='w', textvariable=self.output_filepath, style='filepath.TLabel', padding='0 5 0 5')
        outputButtonFrame = ttk.Frame(contentFrame, padding='0 5 0 0')
        saveButton = Button(outputButtonFrame, text="Save As",
    ...
    def initialise(self):
        ...
        filter_standards_page.footerFrame.set_prev_page(
            self.title + InputOutputPage.__name__)
